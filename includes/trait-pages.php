    ...
    public function add_help_tab() {
        ...
        $screen->add_help_tab( array(
            ...
            'content' =>
                ...
                '<p>' . __( '<strong>REST API</strong> <em>(Pro)</em> — full <code>scriptomatic/v1</code> REST API (WordPress Application Passwords). All history endpoints are read-only via the API (<code>/history</code>, <code>/urls/history</code>, <code>/prefs/history</code>). Activity-log clear/delete is available only from the Dashboard and WP-CLI — not the API. <strong>WP-CLI</strong> <em>(Pro)</em> — <code>wp scriptomatic</code> command group. Both share the same validation and activity logging as the admin UI. Three API access controls in <em>Preferences</em>: enable/disable toggle, IP allowlist, and allowed-users list. <strong>Preferences management</strong> (<code>wp scriptomatic prefs get</code>, <code>prefs set</code>, <code>prefs history</code>) is available via the Dashboard and WP-CLI — intentionally absent from the REST API.', 'scriptomatic' ) . '</p>' .
                '<p>' . __( '<strong>Email Notifications</strong> — administrators opt in via their WordPress profile page to receive a plain-text email on every script save, rollback, URL change, file event, and restore.', 'scriptomatic' ) . '</p>' .
                ...
                '<p>' . __( 'This plugin is designed with security and performance in mind, providing input validation, sanitisation, secondary nonce verification, an activity log with revision rollback, and conditional loading.', 'scriptomatic' ) . '</p>',
        ) );
        ...
        $screen->add_help_tab( array(
            ...
            'content' =>
                ...
                '<p><strong>' . __( 'Preferences via WP-CLI (Pro):', 'scriptomatic' ) . '</strong> ' . __( 'Read plugin preferences with <code>wp scriptomatic prefs get</code>; update a single value with <code>wp scriptomatic prefs set --key=&lt;key&gt; --value=&lt;value&gt;</code>; review the change history with <code>wp scriptomatic prefs history</code>. Valid keys: <code>max_log_entries</code> (3&ndash;1000), <code>keep_data_on_uninstall</code> (bool), <code>save_confirm_enabled</code> (bool); Pro-only: <code>api_enabled</code> (bool), <code>api_allowed_ips</code> (IPv4/IPv6/CIDR), <code>api_allowed_users</code> (admin logins or IDs). Preferences management is available from the Dashboard and WP-CLI — not the REST API. Requires a Pro licence.', 'scriptomatic' ) . '</p>' .
        ...
        $screen->add_help_tab( array(
            ...
            'content' =>
                ...
                '<li>' . __( 'Check if another plugin is conflicting (try disabling other plugins temporarily).', 'scriptomatic' ) . '</li>' .
                '</ul>' .
