    ...
    public function add_help_tab() {
        ...
        $screen->add_help_tab( array(
            ...
            'content' =>
                ...
                '<p>' . __( '<strong>REST API</strong> <em>(Pro)</em> — full <code>scriptomatic/v1</code> REST API (WordPress Application Passwords). All history endpoints are read-only via the API (<code>/history</code>, <code>/urls/history</code>, <code>/prefs/history</code>). <strong>WP-CLI</strong> <em>(Pro)</em> — <code>wp scriptomatic</code> command group. Both share the same validation and activity logging as the admin UI. Three API access controls in <em>Preferences</em>: enable/disable toggle, IP allowlist, and allowed-users list. <strong>Preferences management</strong> (<code>wp scriptomatic prefs get</code>, <code>prefs set</code>, <code>prefs history</code>) is available via the Dashboard and WP-CLI — intentionally absent from the REST API.', 'scriptomatic' ) . '</p>' .
                '<p>' . __( '<strong>Email Notifications</strong> — the site admin always receives a plain-text email on every script save, rollback, URL change, file event, preferences save, and restore. Other administrators can opt in via their WordPress profile page.', 'scriptomatic' ) . '</p>' .
